...
function chart_bar_stacked() {
	var config = _bar_stacked_chart_config();
...
/**
 * internal _bar_stacked_chart_config()
 */
function _bar_stacked_chart_config() {
	var config = {
	...
	config.drawChart = function(chartContainerID) {
		...
		// ---------------------------------
		config.ymax = 0;
		config.ylegendMaxLen = 0;
		config.dataLayers = config.ykeys.map(function(name, i) {
			var l = name.length;
			if(l > config.ylegendMaxLen) { config.ylegendMaxLen=l; }
			return config.data.map(function(d) {
				var yvalue = +d[name];
				if(yvalue > config.ymax) {
					config.ymax = yvalue; 
				};
		...
		config.yStackMax = d3.max(config.stackedData, function(data) { 
							return d3.max(data, function(d) { return d.y0 + d.y; }); });
		max = config.yStackMax;
		// ----------------------------------
	...
	config._drawBar = function(p,q,parent)  {
		...
		if(config.tooltip != null) {
			bar.on("mouseover",function(d,i) {
				content = d.x + '<br/>' + config.legends[q]  + ' = <b>' + d.y + '</b>';
		...
		if(config.clickAction != null) {
			bar.on("click",function(d,i) {
				...
				if (actionType == 'callback') {	
					eval( actionTarget+"(d.x,d.y,config.legends[q])" );
			    } else {
